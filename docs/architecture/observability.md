...
sequenceDiagram
    ...
    Client->>PayAPI: POST /transactions (X-Correlation-Id, optional)
    Note over PayAPI: 1. Generated if absent. Held for<br/>the life of the request only
    PayAPI->>PayDB: INSERT outbox_events (correlation_id)
    Note over PayDB: 2. Written down, because the<br/>request ends before the send

    PayDB-->>Poller1: Poll outbox_events
    Poller1->>Kafka: Produce + header X-Correlation-Id
    ...
    Ledger->>LedgerDB: INSERT entries + outbox_events (correlation_id)
    Note over LedgerDB: 5. Same id continues onto the<br/>ledger's own outgoing message

    LedgerDB-->>Poller2: Poll ledger outbox
    Poller2->>Kafka: Produce + header X-Correlation-Id
    Kafka->>PayAPI: Consume ledger-events
    PayAPI->>PayDB: UPDATE status
...
Hops 2 and 3 are the whole reason this design exists.
